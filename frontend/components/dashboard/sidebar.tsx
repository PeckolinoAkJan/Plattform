...
const NAV_ITEMS = [
  ...
  { href: '/dashboard/map', label: 'Live-Karte', icon: '🛰️' },
  { href: '/dashboard/experiments', label: 'Experimente', icon: '🧪' },
];
...
export default function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const slideClass = mobileOpen ? 'translate-x-0' : '-translate-x-[calc(100%-4.5rem)]';
      ...
      >
        ...
        <div className="mb-6 flex items-center justify-between border-b border-gold-800/50 pb-3">
          <div className="min-w-0">
            {!collapsed ? (
              <BrandMark size={36} withWordmark variant="logo" ariaLabel="VTC Hub" />
            ) : (
              <BrandMark size={28} withWordmark={false} variant="logo" ariaLabel="VTC Hub" />
            )}
          </div>
          <button
            onClick={onMobileClose}
