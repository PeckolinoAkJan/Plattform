...
export default function CompanyPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setDraftCountryCode((next as { countryCode?: string | null }).countryCode ?? '');
      } catch {
        setMessage('Aktuell ist noch keine Spedition zugeordnet oder kein Zugriff möglich.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (draftCountryCode.trim()) {
        (payload as { countryCode?: string }).countryCode = draftCountryCode.trim().toUpperCase();
      }
  ...
  return (
    <main className="space-y-4">
      <ScrollReveal>
        <div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-gold-300">Organisation</p>
        ...
        <>
          ...
          <ScrollReveal delayMs={80}>
            <section className="rounded-2xl border border-gold-700/55 bg-ink-900/70 p-5">
              <div className="mb-4 flex flex-wrap items-center gap-4 border-b border-gold-700/45 pb-4">
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.25em] text-gold-300">Aktive Spedition</p>
                  <h2 className="text-xl font-semibold text-white">{company.name || 'Keine Spedition'}</h2>
                  <p className="text-sm text-gold-200/85">{company.tag || '—'}</p>
                </div>
              </div>
          ...
          <ScrollReveal delayMs={130}>
            <form onSubmit={onSubmit} className="grid gap-3 rounded-2xl border border-gold-700/55 bg-ink-900/70 p-5">
              <h2 className="text-sm uppercase tracking-[0.3em] text-gold-200">Speditionsdaten bearbeiten</h2>

              <label className="block text-sm">
                <span className="mb-1 block text-xs uppercase tracking-[0.2em] text-gold-300/80">Name</span>
                <input
                  value={draftName}
              ...
              <label className="block text-sm">
                <span className="mb-1 block text-xs uppercase tracking-[0.2em] text-gold-300/80">Tag</span>
                <input
                  value={draftTag}
              ...
              <label className="block text-sm">
                <span className="mb-1 block text-xs uppercase tracking-[0.2em] text-gold-300/80">Land / Code</span>
                <input
                  value={draftCountryCode}
                  onChange={(event) => setDraftCountryCode(event.currentTarget.value)}
                  maxLength={3}
                  className="w-full rounded-lg border border-gold-700/50 bg-ink-950/70 px-3 py-2 text-sm text-white"
              ...
              <label className="block text-sm">
                <span className="mb-1 block text-xs uppercase tracking-[0.2em] text-gold-300/80">Beschreibung</span>
                <textarea
