...
export class SteamStrategy extends PassportStrategy(SteamStrategyBase, "steam") {
  constructor(
    ...
    const steamApiKey = configService.get<string>("STEAM_API_KEY") || "oauth-disabled";
    ...
    const realm = configService.get<string>("STEAM_REALM") ?? new URL(returnURL).origin;
  ...
  async validate(identifier: string, profile: Record<string, any>): Promise<any> {
    ...
    const userProfile: SocialAuthProfile = {
      ...
      email: null,
      emailVerified: false,
      displayName: profile.personaname || profile.displayName || profile.steamid || steamId,
  ...
  private extractSteamId(identifier: string, profile: Record<string, any>): string {
    return profile.steamid || profile.id || identifier.split("/").at(-1) || identifier;
  }
}
